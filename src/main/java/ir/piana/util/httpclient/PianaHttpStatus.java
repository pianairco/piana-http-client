...
public enum PianaHttpStatus {
    ...
    PianaHttpStatus (final int statusCode, final String reasonPhrase) {
     ...
     */
    public static PianaHttpStatus fromStatusCode(final int statusCode)
            throws PianaHttpClientException {
        for (PianaHttpStatus s : PianaHttpStatus.values()) {
            if (s.code == statusCode) {
                return s;
            }
        }
        throw new PianaHttpClientException(
                PianaRestException.AMBIGUOUS_STATUS,
